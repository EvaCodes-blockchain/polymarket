...
| [team-composition.md](team-composition.md)                                                     | The largest team (48 seats) that can work in parallel without blocking: ownership boundaries per component, frozen-contract rules, Phase-0 bootstrap, dependency map, MVP critical-path subset, and the maximality argument. The working agent team in `.claude/agents/` instantiates its developer seats (Groups B–F + `dev-lead`).  |
| [team-architects-backup.md](team-architects-backup.md)                                         | Archive of the retired five-agent architects team (chief-architect, blockchain-architect, component-repo-architect, test-strategy-architect, architecture-reviewer): verbatim agent definitions, restoration instructions, and the mapping of each charter to its inheritor in the developer team.                                  |
