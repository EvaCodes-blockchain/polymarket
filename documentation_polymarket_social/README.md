...
| [ui-testing.md](testing-ui)                                                                 | End-to-end UI testing strategy: Playwright setup, injected test wallet, page objects, critical user journeys (J1–J8), component-behavior specs, browser/viewport matrix.                                                      |
...
| [architecture-golang.md](architecture-golang.md)                                              | The **Chain Event Indexer** — a small Go service that is the single bridge from on-chain events (`MarketCreated`, `Trade`, `MarketResolved`, `Redeemed`) to the off-chain read path. Built on [ethbacknode](https://github.com/ITProLabDev/ethbacknode) (node monitoring sidecar) and [go_pysyun_pipeline](https://github.com/pysyun/go_pysyun_pipeline) (stage composition): pipeline design, delivery guarantees, schema, sync endpoint, failure modes, Base migration notes. |
| [reference-upstream-go-components.md](architecture-golang-components)                     | Pinned technical reference (snapshot 2026-06-11) for the two upstream Go components the Chain Event Indexer builds on: ethbacknode (JSON-RPC methods, `blockEvent`/`transactionEvent` callback payloads, config, deployment constraints, license caveats) and go_pysyun_pipeline (public API, behavioral guarantees, error/backpressure limitations). |
