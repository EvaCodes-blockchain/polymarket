...
describe("MVP Buy Flow", function () {
  ...
  // ─── Per-market contracts (created in beforeEach) ──────────────────
  ...
  beforeEach(async function () {
    ...
    acl = (await ACLFactory.deploy(deployer.address)) as JustifyAccessControl;

    // ── Deploy MockUSDC ─────────────────────────────────────────────────
    const USDCFactory = await ethers.getContractFactory("MockUSDC", deployer);
    usdc = (await USDCFactory.deploy(deployer.address)) as MockUSDC;

    // ── Deploy OutcomeToken ─────────────────────────────────────────────
    const OTFactory = await ethers.getContractFactory("OutcomeToken", deployer);
    outcomeToken = (await OTFactory.deploy(deployer.address)) as OutcomeToken;

    // ── Deploy FeeTreasury ──────────────────────────────────────────────
    const TreasuryFactory = await ethers.getContractFactory("FeeTreasury", deployer);
    feeTreasury = (await TreasuryFactory.deploy(
    ...
    oracleResolver = (await ORFactory.deploy(await acl.getAddress())) as OracleResolver;

    // ── Deploy MarketFactory ────────────────────────────────────────────
    const MFFactory = await ethers.getContractFactory("MarketFactory", deployer);
    factory = (await MFFactory.deploy(
      await acl.getAddress(),
    ...
    // Grant FACTORY_ROLE and admin on OutcomeToken to the factory contract
    await acl.grantRole(await acl.FACTORY_ROLE(), await factory.getAddress());
    // Factory needs DEFAULT_ADMIN_ROLE on OutcomeToken to grant MINTER_ROLE to AMMs
    const adminRole = await outcomeToken.DEFAULT_ADMIN_ROLE();
    await outcomeToken.grantRole(adminRole, await factory.getAddress());
    ...
    market = (await ethers.getContractAt("PredictionMarket", marketAddr)) as PredictionMarket;
    amm = (await ethers.getContractAt("MarketAMM", ammAddr)) as MarketAMM;
  ...
  describe("Pool seeding", function () {
    ...
    it("YES implied probability ≈ 40% (within 1 bps)", async function () {
    ...
    it("NO implied probability ≈ 60% (within 1 bps)", async function () {
  ...
  describe("CEO-4: Buy flow", function () {
    ...
    it("emits Buy event with correct fields", async function () {
      const ammAddr = await amm.getAddress();
      await usdc.connect(trader).approve(ammAddr, BUY_AMOUNT);
    ...
    it("mints ERC-1155 YES shares to trader after buy", async function () {
      ...
      // Trader received non-zero YES shares
    ...
    it("mints NO shares to trader when buying NO", async function () {
      ...
      const balanceAfter = await outcomeToken.balanceOf(trader.address, noTokenId);
      expect(balanceAfter - balanceBefore).to.be.gt(0n);
    });
    ...
    it("updates reserves after buy", async function () {
      ...
      const [r0Before, r1Before] = await Promise.all([amm.reserves(0n), amm.reserves(1n)]);

      await amm.connect(trader).buy(0, BUY_AMOUNT, 0n); // buy YES

      const [r0After, r1After] = await Promise.all([amm.reserves(0n), amm.reserves(1n)]);
    ...
    it("multiple buys accumulate ERC-1155 balance", async function () {
      const ammAddr = await amm.getAddress();
      await usdc.connect(trader).approve(ammAddr, BUY_AMOUNT * 3n);
